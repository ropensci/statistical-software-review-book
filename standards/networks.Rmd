...
The following standard reflects one aim of rOpenSci to curate an interoperable "ecosystem" of
software:
...
- **NW2.3** *Network software which uses attributes of input data such as row       <!-- This seems a little obscure, should it be general or have an example? --> 
...
- **NW2.4** *Network software for which input data does not generally include    <!-- Again, a bit difficult to follow, does this mean auto-number labelling -->
...
- **NW2.5** *Network Software should transfer all relevant aspects of input      <!-- I think this should be the first of these three, and maybe add a heading on "Labels" and an overall sentence on the logic of threse three together --> 
...
- **NW2.10** *This pre-processing function should maintain metadata attributes
  ...
  values. *  <!-- This is closely related to the data labelling comments above.  Maybe it should go first, and then you can get into details of labels. Are there are metadata types that we should mention? -->
...
- **NW4.6** *Grouping algorithms which apply sequential labels should ensure
  ...
  on.)*  <!-- I don't agree with this, because there are other ways the ordering/labeling issue should be solved.  However, the software should document how the ordering of groups is determined, whether and how the order is deterministic and how users can set parameters or seeds to have stable labels -->
...
- **NW4.7** *Dimensionality reduction or equivalent algorithms which label
  ...
  “importance” (for example, eigenvalues or variance contributions).*  <!-- Same as above -->
