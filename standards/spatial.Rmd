...
- **SP2.3** *Software which accepts spatial input data in any standard format
  ...
  test code which load those data in original spatial formats.*
...
- **SP2.4** *Geographical Spatial Software should be compliant with version 6
  (and, ideally 7) of* [`PROJ`](https://proj.org/), *and with* `wkt2`
  *representations. The primary implication, described in detail in the articles
  linked to above, is that:*
...
- **SP2.7** *Spatial Software should accept input data in as many specific
  spatial classes as possible.*
- **SP2.8** *Spatial Software should implement validation routines to confirm
...
- **SP2.9** *Spatial Software should implement a single pre-processing routine
...
- **SP2.10** *The pre-processing function described above should maintain those
...
-   **SP2.11** *Spatial software should accept inputs defined via the* [`units`
...
test and training data [@muenchow_chapter_nodate;@brenning_spatial_2012;@schratz_hyperparameter_2019;@valavi_blockcv_2019].
