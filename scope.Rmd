...
rather than having using other formal criteria. We use a mix of empirical on
...
testing](#framework-testing), like the [`hypothesis` framework for
...
***Proposal*** For packages which parameterise or fit probability
distributions, develop routines to assess and quantify the sensitivity of
outputs to the distributional properties of inputs, and particularly to
deviations from assumed distributional properties.
...
1. We seek explicit advise and expertise from somebody with domain expertise on
   how we might best approach assessing software for categorical variables.
2. We make this category something of an exception in that we will attempt to
   develop approaches to standardising assessment in response to any initial
   submissions we might receive. We would certainly explicitly acknowledge the
   "trial" nature of this category, and might also productively ask authors of
   initial packages for constructive input on how this might be achieved.
3. In the absence of any category-specific standards which may arise or
   otherwise be deemed applicable, this software ought be assessed under all
   applicable general categories, with other aspects to be *qualitatively*
   assessed by editors and reviewers. (This category is the single one, after
   all, which primarily deals with *qualitative* data.)
...
1. We identify a sub-category of software which accepts *network inputs*, and
   develop (or adapt) general techniques to generate generic graphs to be used
   in benchmarking routines.
2. Other software which falls within this category only because of restricted
   aspects such as internal data representations (such as `tcherry`) *not* be
   considered or assessed within this category.
...
1. [`survxai`](https://joss.theoj.org/papers/10.21105/joss.00961)
