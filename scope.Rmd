...
"Methods and Algorithms" comprise the core of topics addressable in this
project. The primary empirical data used to inform the following categorisation
are represented in the [interactive
...
and JOSS. The primary nodes that emerge from those analyses (with associated
*relative* sizes in parentheses) are:
...
terms <- data.frame (n = seq_along (terms),
                     term = names (terms),
                     prop = as.integer (terms) / n_abstracts)
knitr::kable (head (terms, n = 15), digits = c (0, NA, 3), caption = "Most frequent key words from all JOSS abstracts for statistical software. Proportions (\"*prop*\") are scaled *per abstract*, with each abstract generally having multiple key words, and so sum of proportions exceeds one.")
```

The following lists some of the primary categories which might plausibly be
distinguished from the these terms, and the inter-relationships with the main
[network
diagram](https://ropenscilabs.github.io/statistical-software/abstracts/network-terms/index.html).
We have excluded the key word "Estimates" as being too generic to usefully
inform standards, and we have also collected a few strongly-connected terms
into single categories. Importantly, these categories are not mutually
exclusive, and much software will likely be described by multiple categories.

1.  **Bayesian & Monte Carlo (0.167)**
2.  **Dimensionality Reduction & Feature Selection** (commonly as a result of
    ML algorithms; **0.155**)
3.  **Machine Learning (ML; 0.133)**
4.  **Regression/Splines/Interpolation (0.122)** including function data analysis
5.  **Probability Distributions (0.100)** include kernel densities, likelihood
    estimates and estimators, and sampling routines
6.  **Categorical Variables (0.078)** including latent variables, and those output from
    ML algorithms
7.  **Networks (0.078)**
8. **Exploratory Data Analysis (EDA; 0.067)** including information statistics
   such as Akaike's criterion, and techniques such as random forests. Often
   related to workflow software.
9. **Survival (0.067)** Strongly related in the empirical data to regression
   analysis, yet clearly a category of its own.
10. **Summary Statistics (0.056)** Primarily categorising software for which
    the major aim is to generate summary statistics of otherwise complex
    outputs. Also strongly related to EDA, yet differing in being strictly
    descriptive of software *outputs* whereas EDA may include routines to
    explore data *inputs* and other pre-output stages of analysis.
11. **Visualization (0.056)** Primarily related in the empirical data to
    regression and survival analyses, yet clearly a distinct category of its
    own.
12. **Workflow (0.056)** Often related to EDA, and very commonly also to ML.
13. **Model Selection (0.044)** A important intermediate node between such
    categories as ML, Bayesian and Monte Carlo techniques, visualisation, and
    EDA.
14. **Spatial (0.033)** Also an important intermediate node connecting several
    other nodes, yet defining its own distinct cluster reflecting a distinct
    area of expertise.
...
ns <- ns [c (1:5, 7:13, grep ("spatial", ns$label),
             grep ("model selection", ns$label)), ]
es <- edges
for (i in seq (nrow (ns))) {
    lab <- ns$label [i]
    dgi <- match (lab, rownames (dg))
    nbs <- rownames (dg) [which (dg [dgi, ] < 3)]
    index <- which ((edges$from == lab & edges$to %in% nbs) |
                    (edges$to == lab & edges$from %in% nbs))
    ns$value [i] <- sum (edges$width [index])
    index <- which (edges$from == lab & edges$to %in% nbs)
    es$width [index] <- sum (edges$width [index])
}
