...
submission, to tools for identifying and assigning reviewers, to methods and
tools for structured review interventions after software has been officially
accepted. Although there are several potential general models we could adapt
for our proposed system, we anticipate the general workflow being based on
[github](https://github.com), for which two of the most prominent current
models are rOpenSci's own [submission
...
these two systems, and so they provide convenient reference points in
considering many of the following phases we envision developing. It is
nevertheless important to keep in mind that these examples are based on only
two systems, and there may be many alternative ways of approaching the kinds of
issues considered throughout the following phases we envision to comprise our
system of peer review for statistical software.
...
whereas the both pre-submission and submission enquiries to the JOSS are
...
1. rOpenSci's pre-submission enquiries are entirely optional, whereas initial
   submissions to the JOSS are by default pre-submission enquiries (unless
...
1.  All submissions be initiated as mandatory pre-submission enquiries which
    ...
    focussed. It also adds clarity through removing potential ambiguity in
    deciding between two distinct ways to commence submission.
2. The process of pre-submissions be partially automated in a similar manner to
   the current system of the JOSS, with metrics extended and adapted to the
   unique needs of our own project.
3. Automation procedures may perhaps be extended by some form of automated
   identification or suggestion of appropriate reviewers.
...
1. Progression from pre-submission to submission be automated as for the JOSS.
...
assignment of due dates for submission of reviews. The JOSS imposes a generic
...
The review processes of rOpenSci and the JOSS are qualitatively different, with
...
bottom of a github issue. This approach will face on immediate difficulty in
that templates will likely differ for different kinds of statistical software,
through software being described by different combinations of the [categories
described above](#scope). It may suffice to combine a generic "master" template
with category-specific items to be appended according to the description of
submitted software within our list of categories, although it is important to
note that this would exclude review criteria reflecting unique combinations of
categories (for example, a checklist item appropriate for the visualisation of
results from ML algorithms).
...
- [ ] Have automated tests (considered by the JOSS as part of "Documentation")

In addition, the JOSS requires reviewers:
