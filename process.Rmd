...
developing. It is nevertheless important to keep in mind that the following
examples generally refer to these two systems only, and that there may be many
alternative ways of approaching the kinds of issues considered throughout the
following phases we envision to comprise our system of peer review for
...
and JOSS whereby authors can enquire about whether a potential submission is
...
2. The process of pre-submission be partially automated in a similar manner to
   ...
   pre-submission stage will be generated, as for example the JOSS system of
...
line-by-line, currently amounting to 1,163 names, liking making it not
...
[airtable](https://airtable.com). The latter is not ideal, because the first
suggested login option, via a google account, automatically grants google
access to all of a user's personal contacts, an invasion of privacy to which
many may understandably object. While it is possible to view JOSS's google
document anonymously, anybody who happens to be simultaneously logged in to
a google account --- notably including by default all Android users --- has
already assented to google logging all interactions with the table. The mere
possibility of anonymous viewing nevertheless makes JOSS's approach better in
many ways, yet illustrates the conflicts inherent in curating such databases of
personal information, through all personal information within the table being
