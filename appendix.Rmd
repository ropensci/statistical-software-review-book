...
2. Create new badges in [the `svgs` directory of
   ...
   by coping existing versions, and updating the text for the version numbers.
   These should only have major and minor versions, "XX.YY". Commit and push
   changes to that repository.
3. Ensure that corresponding labels exist on [the main `software-review`
   repository](https://github.com/ropensci/software-review/labels), or create
   labels with new version numbers if they don't exist.
...
the `ropensci-review-bot` then use the external endpoint, `stats_badge`,
