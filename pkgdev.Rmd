...
Badge grades may also be downgraded for code which is not updated to match
ongoing developments in standards. The following sub-sections provide further
clarification of each grade.
...
developers do not intend to develop further following review. This commonly
...
that software has been developed for a particular use case, with developers
...
requirements of bronze in at least one the follows aspects:
...
- Demonstrating excellence in compliance with multiple standards from at least
  ...
  6.1.5. This aspect would require software to extend notable beyond the
  requirements of two or more standards in at least two sub-categories
  (regardless of whether of general of category-specific standards). Software
  which might otherwise be assessed at bronze grade, yet which is both
...
- Have a demonstrated *generality* of usage beyond one single envisioned use
  case. Software is frequently developed for a particular use case envisioned
  by the developers themselves. Generalising the utility of software so that it
  is readily applicable to other use cases, and satisfactorily documenting such
  general abilities, represents another aspect which may be considered
  sufficient for software to attain a silver grade.

To attain a silver grade, software should fulfil at least one of these three
aspects. Other factors may also be taken into account at the discretion of
reviewers, such as exemplary package design or algorithmic implementation.


### Gold

As stated, a gold badge is granted to software which complies with all
applicable standards, and which generally fulfils all of the three aspects
described above for silver-grade badges. Both the applicability of standards,
and fulfilment of these three aspects, will ultimately determined by reviewers.
Moreover, compliance with all grades is assessed against current standards,
meaning that a gold badge must be actively maintained as standards themselves
